  ...
  return (
    <AuthGuard adminOnly>
      <div className="min-h-screen app-bg flex">
        <aside className="w-80 shrink-0 border-r border-slate-200/80 bg-white/80 backdrop-blur-xl flex flex-col sticky top-0 h-screen">
          <div className="p-7 border-b border-slate-100 flex items-center gap-3.5">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/images/logo.png" alt="Master SAT" className="w-11 h-11 object-contain" />
            <div>
              <p className="text-lg font-extrabold text-slate-900 tracking-tight leading-tight">Teacher Panel</p>
            ...
            {nav.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={href}
                  href={href}
                  className={`flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-base font-semibold transition-all ${
                    active
