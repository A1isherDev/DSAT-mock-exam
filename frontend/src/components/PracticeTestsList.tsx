...
export default function PracticeTestsList({
  ...
  const filtered = useMemo(() => {
    ...
    return tests.filter((t: any) => {
      const modulesBlob = (t.modules || [])
        .map((m: any) => `module ${m.module_order} ${m.time_limit_minutes}`)
        .join(" ");
      const blob = `${subjectLabel(t.subject)} ${t.label || ""} ${t.form_type || ""} ${t.mock_exam?.title || ""} ${modulesBlob}`.toLowerCase();
    ...
    <div className="max-w-7xl mx-auto px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map((t: any) => {
          ...
          const isRW = t.subject === "READING_WRITING";

          return (
            <div
              key={t.id}
              className={`group bg-white dark:bg-slate-900 rounded-[32px] shadow-sm overflow-hidden flex flex-col border-2 transition-all duration-500 ${
                isRW
                  ? "border-blue-50 dark:border-blue-900/40 hover:border-blue-400"
                  : "border-emerald-50 dark:border-emerald-900/40 hover:border-emerald-400"
              } hover:shadow-2xl hover:shadow-slate-200/50 dark:hover:shadow-emerald-950/20`}
            >
              <div className="p-8 pb-4 relative">
                ...
                <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/40 px-4 py-3">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Tests in this card</p>
                  {modules.length === 0 ? (
                    <p className="text-xs font-medium text-slate-400">No modules yet</p>
                  ) : (
                    <ul className="space-y-2">
                      {modules.map((m: any) => (
                        <li
                          key={m.id}
                          className="flex items-center justify-between gap-2 text-sm font-bold text-slate-700 dark:text-slate-200"
                        >
                ...
                {completed && (
                  <p className="mt-4 text-[10px] font-black text-emerald-600 uppercase tracking-widest">Completed</p>
                )}
              </div>
