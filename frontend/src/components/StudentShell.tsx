  ...
  return (
    <AuthGuard isOptional>
      <div className="min-h-screen app-bg dark:bg-[#020617] transition-colors duration-300 flex text-slate-900 dark:text-slate-100">
        <aside className="w-80 shrink-0 border-r border-slate-200/80 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl flex flex-col sticky top-0 h-screen transition-colors duration-300">
          <div className="p-7 border-b border-slate-100 dark:border-slate-800/50 flex items-center gap-3.5">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/images/logo.png" alt="Master SAT" className="w-11 h-11 object-contain" />
            <div>
              <span className="block text-lg font-extrabold text-slate-900 dark:text-white tracking-tight">MasterSAT</span>
            ...
            {nav.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  key={href}
                  href={href}
                  className={`flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-base font-semibold transition-all duration-200 ${
                    active
                ...
                >
                  <Icon className={`w-5 h-5 shrink-0 ${active ? "opacity-100 text-blue-600 dark:text-blue-400" : "opacity-70"}`} />
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="h-[74px] border-b border-slate-200/80 dark:border-slate-800 bg-white/75 dark:bg-slate-950/80 backdrop-blur-xl flex items-center justify-end px-10 gap-4 shrink-0 sticky top-0 z-40 transition-colors duration-300">
            {mounted && (
              <button
