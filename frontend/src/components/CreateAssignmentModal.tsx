...
type AssignmentOptModule = { id: number; module_order: number };
...
export default function CreateAssignmentModal({ open, classId, onClose, onSuccess }: Props) {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setAsgOptionsError(typeof msg === "string" ? msg : "Ro‘yxatlarni yuklab bo‘lmadi.");
        }
      } finally {
  ...
  const selectedPracticeTest = assignmentOptions.practice_tests.find((p) => String(p.id) === newAsg.practice_test);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setFormError(typeof d === "string" ? d : "Topshiriq yaratilmadi.");
    } finally {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20"
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between gap-4 px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white rounded-t-3xl">
          <div>
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Yangi topshiriq</p>
        ...
        <div className="p-6 space-y-4">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            <strong>Mock</strong> — vaqt bilan diagnostik imtihon. <strong>Pastpaper</strong> — alohida mashq (leaderboard
            statistikasi uchun). Hammasi ixtiyoriy; sarlavha majburiy.
          </p>

          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Sarlavha *</label>
            <input
              value={newAsg.title}
              onChange={(e) => setNewAsg((p) => ({ ...p, title: e.target.value }))}
              placeholder="Masalan: May SAT Reading mashqi"
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Ko‘rsatmalar</label>
            <textarea
              value={newAsg.instructions}
              onChange={(e) => setNewAsg((p) => ({ ...p, instructions: e.target.value }))}
              placeholder="Talabalar uchun qisqa yo‘riqnoma"
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Muddat (kalendar)</label>
            <input
              type="datetime-local"
              value={dueLocal}
            ...
            />
            <p className="text-[11px] text-slate-400 mt-1">Bo‘sh qoldirsangiz, muddat cheklanmaydi.</p>
          </div>
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Mock imtihon</label>
            <select
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Pastpaper test</label>
            <select
              value={newAsg.practice_test}
              onChange={(e) => {
                const v = e.target.value;
                setNewAsg((p) => {
                  const next = { ...p, practice_test: v };
                  if (!v) {
                    next.module = "";
                    return next;
                  }
                  const pt = assignmentOptions.practice_tests.find((x) => String(x.id) === v);
                  if (pt && p.module && !pt.modules.some((mod) => String(mod.id) === p.module)) {
                    next.module = "";
                  }
                  return next;
                });
            ...
            >
              ...
              {assignmentOptions.practice_tests.map((pt) => (
                <option key={pt.id} value={pt.id}>
                  {pt.title}
                  {pt.practice_date ? ` · ${pt.practice_date}` : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Modul (ixtiyoriy)</label>
            <select
              value={newAsg.module}
              onChange={(e) => setNewAsg((p) => ({ ...p, module: e.target.value }))}
              disabled={!selectedPracticeTest || selectedPracticeTest.modules.length === 0}
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-medium bg-white focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 outline-none disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="">— Barcha modullar —</option>
              {(selectedPracticeTest?.modules || []).map((mod) => (
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Fayl (ixtiyoriy)</label>
            <input
              type="file"
